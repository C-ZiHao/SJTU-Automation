...
def run_find_tag(img_queue, tag_queue, display=False):
    m = cv2.dnn.readNetFromTensorflow("model.pb")
...
def run_find_obs(img_queue, obs_queue, display=False):
    m = cv2.dnn.readNetFromTensorflow("obs_model.pb")
    x = np.zeros([1,3,32,32],dtype=np.float32)
    m.setInput(x)
    m.forward()
    while True:
        ...
        obs_queue.put((None if obs is None else obs[0], img if display else None))


def line_patrol(run=True, display=False, record=False, with_tag=True, with_obs=True):
    tag_img_queue = Queue()
    tag_rst_queue = Queue()
    obs_img_queue = Queue()
    obs_rst_queue = Queue()
    ...
    while True:
        ...
        if not obs_rst_queue.empty():
            ...
            if obs == "left":
                ...
                obs_miss_cnt = 0
        # 正常巡线
        base_speed = 10 if tag_running else 120
...
if __name__ == "__main__":
    line_patrol(run=False, display=False, record=False, with_tag=True, with_obs=True)
